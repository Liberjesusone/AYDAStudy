#pragma once
#pragma GCC diagnostic ignored "-Wunused-parameter" //Recordar Borrar para ignorar advertencia
#include <graph.hpp>
...
public:
    ...
    {
       // Nota: En tu librería el tipo para grafos sin peso suele ser NoWeight, 
        // ajustamos el assert para que haga match con tu arquitectura.
       static_assert(!std::is_same_v<WeightType, NoWeight>, "A* requires a weighted graph.");

        // 1. Estructuras de control
        std::unordered_map<NodePtr, NodePtr> came_from;    // Para reconstruir el camino al final
        std::unordered_map<NodePtr, WeightType> g_score;   // Costo real desde el inicio
        
        // Min-Heap (Cola de prioridad) ordenada por el f_score (Costo estimado total)
        using Par = std::pair<WeightType, NodePtr>;
        std::priority_queue<Par, std::vector<Par>, std::greater<Par>> open_set;

        // 2. Inicializamos el nodo de origen
        g_score[start] = WeightType{}; // Equivale a 0
        open_set.push({heuristic(start), start}); // Su f_score inicial es solo la heurística

        // 3. Comenzamos la exploración
        while (!open_set.empty()) 
        {
            // ¡ARREGLO PARA C++17!
            // Extraemos los elementos del std::pair de forma clásica para evitar 
            // problemas de captura de Structured Bindings dentro de la lambda.
            auto top_element = open_set.top();
            //WeightType f_actual = top_element.first;
            NodePtr actual = top_element.second;
            
            open_set.pop();

            // ¡Llegamos a la meta! Reconstruimos el camino hacia atrás
            if (actual == goal) 
            {
                ...
                {
                    path.push_back(paso);
                    if (came_from.find(paso) != came_from.end()) {
                        paso = came_from[paso];
                    } else {
                        paso = nullptr; // Llegamos al inicio
                    }
                }
            ...
            // --- Sub-función (Lambda) para procesar a los vecinos limpiamente ---
            auto procesar_vecino = [&](NodePtr vecino, WeightType peso_arco) {
                WeightType g_tentativo = g_score[actual] + peso_arco;

                // Si encontramos un camino más corto hacia este vecino (o es la primera vez que lo vemos)
                if (g_score.find(vecino) == g_score.end() || g_tentativo < g_score[vecino]) 
                {
                    came_from[vecino] = actual; // Dejamos la miga de pan
                    g_score[vecino] = g_tentativo;
                    
                    // f_score = Costo Real (g) + Estimación a la meta (h)
                    WeightType f_score = g_tentativo + heuristic(vecino);
                    
                    open_set.push({f_score, vecino});
                }
            };

            // 4. EL RADAR 360° (Para soportar UndirectedGraph y DirectedGraph)
            
            // Revisamos salidas
            for (const auto& arco : graph.getOutgoingArcs(actual)) {
                NodePtr vecino = arco->getTgtNode();
                if (vecino == actual) vecino = arco->getSrcNode(); // Protección de self-loops/memoria
                procesar_vecino(vecino, arco->getWeight());
            }

            // Revisamos entradas
            for (const auto& arco : graph.getIncomingArcs(actual)) {
                NodePtr vecino = arco->getSrcNode();
                if (vecino == actual) vecino = arco->getTgtNode(); // Protección de self-loops/memoria
                procesar_vecino(vecino, arco->getWeight());
            }
        }

        // Si la cola de prioridad se vacía y no retornamos arriba, no existe un camino
        return {};
...
public:
     ...
     */
    static GraphType getMinimumPathsTree(const GraphType& graph, NodePtr source, std::vector<NodePtr>& cycle)
    {
        static_assert(!std::is_same_v<WeightType, void>, "Bellman-Ford requires a weighted graph.");
        static_assert(!std::is_same_v<WeightType, NoWeight>, "Error: Bellman-Ford requiere un grafo ponderado.");
        ...
        for (const auto& nodo : graph.getNodes()) {
            tabla[nodo] = {INFINITO, nullptr, nullptr};
        }
        tabla[source] = {WeightType{}, nullptr, nullptr}; 

        size_t V = graph.getNodes().size();
        if (V == 0) return GraphType(); 
        ...
        for (size_t i = 1; i <= V - 1; ++i) {
            ...
            for (const auto& u : graph.getNodes()) {
                if (tabla[u].distance == INFINITO) continue;

                // ¡Ya solo necesitamos getOutgoingArcs gracias al diseño de graph.hpp!
                for (const auto& arco : graph.getOutgoingArcs(u)) {
                    NodePtr v = arco->getTgtNode();
                    if (v == u) v = arco->getSrcNode(); 
                    
                    if (tabla[u].distance + arco->getWeight() < tabla[v].distance) {
                        tabla[v].distance = tabla[u].distance + arco->getWeight();
                        tabla[v].predecessor = u;
                        tabla[v].incoming_arc = arco;
                        hubo_cambios = true;
                    }
                }
            }
        ...
        for (const auto& u : graph.getNodes()) {
            ...
            for (const auto& arco : graph.getOutgoingArcs(u)) {
                ...
                if (tabla[u].distance + arco->getWeight() < tabla[v].distance) {
                    throw std::runtime_error("Fallo Critico: El grafo contiene un ciclo de peso negativo. No existe ruta minima.");
        ...
        for (const auto& [nodo, info] : tabla) {
            if (info.distance != INFINITO) {
                NodePtr nuevo_nodo = arbol_spt.addNode(nodo->getValue());
                mapa_nodos[nodo] = nuevo_nodo;
            }
        }

        for (const auto& [nodo, info] : tabla) {
            if (info.incoming_arc != nullptr) {
                arbol_spt.addArc(mapa_nodos[info.predecessor], mapa_nodos[nodo], info.incoming_arc->getWeight());
            }
        }
...
public:
    ...
    // Estructura auxiliar para la cola de prioridad
    struct AristaFrontera {
        WeightType peso;
        NodePtr origen;
        NodePtr destino;

        // Para que el priority_queue funcione como Min-Heap (menor peso primero)
        bool operator>(const AristaFrontera& otra) const {
    ...
    {
        static_assert(!std::is_same_v<WeightType, NoWeight>, "Error: Prim requiere un grafo ponderado.");

        GraphType mst;
        if (graph.getNodes().empty() || start == nullptr) return mst;

        std::unordered_set<NodePtr> in_mst; // Nodos ya incluidos en el árbol
        std::unordered_map<NodePtr, NodePtr> mapa_nodos; // Diccionario O(1) para clonar nodos
        
        // Min-Heap de aristas (La frontera de expansión)
        std::priority_queue<AristaFrontera, std::vector<AristaFrontera>, std::greater<AristaFrontera>> pq;

        // 1. Inicializamos el nodo de arranque
        in_mst.insert(start);
        mapa_nodos[start] = mst.addNode(start->getValue());

        // Sub-función (Lambda) para escanear el horizonte (Radar 360°)
        auto expandir_frontera = [&](NodePtr u) {
            
            // Revisamos salidas
            for (const auto& arco : graph.getOutgoingArcs(u)) {
                NodePtr v = arco->getTgtNode();
                if (v == u) v = arco->getSrcNode(); // Protección self-loops

                // Solo nos interesan los vecinos que NO han sido conquistados aún
                if (in_mst.find(v) == in_mst.end()) { 
                    pq.push({arco->getWeight(), u, v});
                }
            }
            // Revisamos entradas (Vital para UndirectedGraph)
            for (const auto& arco : graph.getIncomingArcs(u)) {
                NodePtr v = arco->getSrcNode();
                if (v == u) v = arco->getTgtNode();
        ...
        // Escaneamos las primeras aristas desde el inicio
        expandir_frontera(start);

        // 2. Construcción del Árbol (Ciclo Greedy)
        size_t total_nodos = graph.getNodes().size();

        // Seguimos expandiendo hasta que todos los nodos sean parte del árbol
        while (!pq.empty() && in_mst.size() < total_nodos) {
            
            // Extraemos la arista más barata disponible en toda la frontera
            AristaFrontera mejor_arista = pq.top();
            pq.pop();

            NodePtr u = mejor_arista.origen;
            NodePtr v = mejor_arista.destino;

            // Doble chequeo crítico: ¿El destino ya fue agregado por otra ruta más rápida 
            // mientras esta arista esperaba en la cola? Si es así, la ignoramos.
            if (in_mst.find(v) != in_mst.end()) continue;

            // ¡Aprobada! Conquistamos el nuevo nodo y lo agregamos al MST
            in_mst.insert(v);
            mapa_nodos[v] = mst.addNode(v->getValue());

            // Tiramos el cable (Arista) en el nuevo grafo
            mst.addArc(mapa_nodos[u], mapa_nodos[v], mejor_arista.peso);

            // El nuevo nodo ahora escanea su propio horizonte para buscar más vecinos
            expandir_frontera(v);
        }
...
public:
    ...
    /**
     * @brief Calcula el Árbol de Expansión Mínima usando el algoritmo de Kruskal y la estructura DSU.
     */
    static GraphType getMinimumSpanningTree(const GraphType& graph)
    {
        static_assert(!std::is_same_v<WeightType, NoWeight>, "Error: Kruskal requiere un grafo ponderado.");

        GraphType mst;
        if (graph.getNodes().empty()) return mst;

        // 1. ¡LA MEJORA DE RENDIMIENTO O(1)! El diccionario traductor (Viejo -> Nuevo)
        std::unordered_map<NodePtr, NodePtr> mapa_nodos;

        // Copiamos todos los nodos al nuevo grafo y los registramos en el mapa
        for (const auto& nodo : graph.getNodes()) {
            NodePtr nuevo = mst.addNode(nodo->getValue());
            mapa_nodos[nodo] = nuevo; 
        }

        // 2. Extraemos TODAS las aristas y las ordenamos por peso (de menor a mayor - Estrategia Greedy)
        std::vector<ArcPtr> todas_las_aristas;
        for (const auto& arco : graph.getArcs()) {
            todas_las_aristas.push_back(arco);
        }
        ...
        // 3. Usamos nuestro DSU mágico para evitar hacer ciclos
        DSU<GraphType> dsu(graph.getNodes());
        size_t aristas_agregadas = 0;
        size_t nodos_necesarios = graph.getNodes().size() - 1; // Un árbol siempre tiene V-1 aristas
        ...
        for (const auto& arco : todas_las_aristas) {
            ...
            if (dsu.unite(u, v)) {
                ...
                // ¡LA MAGIA O(1)! 
                // Sacamos los punteros clonados directo del mapa en tiempo récord en lugar de iterar
                mst.addArc(mapa_nodos[u], mapa_nodos[v], arco->getWeight());
                
                aristas_agregadas++;

                // Freno de mano: Si ya conectamos todo, no necesitamos evaluar el resto de los cables caros
                if (aristas_agregadas == nodos_necesarios) break;
            }
        }
    ...
    /**
     * @brief Construye un Grafo Completo basado en coordenadas 2D y Distancia Manhattan.
     * Ideal para el problema "Min Cost to Connect All Points" (LeetCode 1584).
     * @param coordinates Vector de pares (x, y) representando los puntos en el plano.
     * @return GraphType Grafo instanciado donde los nodos son los indices de las coordenadas.
     */
    static GraphType buildManhattanGraph(const std::vector<std::pair<int, int>>& coordinates)
    {
        GraphType g;
        std::vector<NodePtr> nodes;

        // 1. Agregar los nodos. Usamos el indice (0, 1, 2...) como ID del nodo.
        for (size_t i = 0; i < coordinates.size(); ++i)
        {
            nodes.push_back(g.addNode(i)); 
        }

        // 2. Conectar todos contra todos calculando el costo Manhattan
        for (size_t i = 0; i < coordinates.size(); ++i)
        {
            for (size_t j = i + 1; j < coordinates.size(); ++j)
            {
                // Fórmula Distancia Manhattan: |x1 - x2| + |y1 - y2|
                int weight = std::abs(coordinates[i].first - coordinates[j].first) + 
                             std::abs(coordinates[i].second - coordinates[j].second);
                
                g.addArc(nodes[i], nodes[j], weight);
            }
        }
...
/**
 * @brief Implementacion de Bellman-Ford usando paralelismo clasico (Hilos y Mutex).
 * Utiliza Cerrojos Granulares (Lock Stripping) para evitar condiciones de carrera.
 */
template <typename GraphType>
class ClassicConcurrentBellmanFord
{
public:
    using NodeType   = typename GraphType::NodeValueType;
    using WeightType = typename GraphType::ArcWeightType;
    ...
    /**
     * @brief Calcula el Arbol de Caminos Minimos paralelizando la relajacion de aristas.
     */
    static GraphType getMinimumPathsTree(const GraphType& graph, NodePtr source, size_t num_threads = std::thread::hardware_concurrency())
    {
        static_assert(!std::is_same_v<WeightType, NoWeight>, "Bellman-Ford requiere un grafo ponderado.");
        ...
        // Doble Buffer: tabla_old (lectura) y tabla_new (escritura)
        std::unordered_map<NodePtr, NodeInfo> tabla_old;
        std::unordered_map<NodePtr, NodeInfo> tabla_new;
        
        // Cerrojos Granulares: Un candado individual por cada nodo
        // Usamos unique_ptr porque std::mutex no se puede copiar en el unordered_map
        std::unordered_map<NodePtr, std::unique_ptr<std::mutex>> cerrojos;

        for (const auto& nodo : graph.getNodes()) {
            tabla_old[nodo] = {INFINITO, nullptr, nullptr};
            tabla_new[nodo] = {INFINITO, nullptr, nullptr};
            cerrojos[nodo] = std::make_unique<std::mutex>();
        }

        tabla_old[source] = {WeightType{}, nullptr, nullptr}; 
        tabla_new[source] = {WeightType{}, nullptr, nullptr}; 

        std::vector<NodePtr> nodos_vec(graph.getNodes().begin(), graph.getNodes().end());

        // Bucle principal de relajacion: V - 1 pasadas
        for (size_t iter = 1; iter <= V - 1; ++iter) 
        {
            // Bandera segura contra hilos para saber si hubo alguna mejora en esta iteracion
            std::atomic<bool> hubo_cambios{false}; 
            
            // Nuestro pool manual de hilos
            std::vector<std::thread> workers;

            size_t chunk_size = (V + num_threads - 1) / num_threads;

            for (size_t t = 0; t < num_threads; ++t) 
            {
                size_t start = t * chunk_size;
                size_t end = std::min(start + chunk_size, V);

                if (start >= end) break; 

                // Lanzamos el hilo de trabajo crudo
                workers.emplace_back([&, start, end]() {
                    for (size_t i = start; i < end; ++i) 
                    {
                        NodePtr u = nodos_vec[i];
                        if (tabla_old[u].distance == INFINITO) continue;
                        ...
                        for (const auto& arco : graph.getOutgoingArcs(u)) 
                        {
                            ...
                            WeightType nueva_distancia = tabla_old[u].distance + arco->getWeight();

                            // 1. Lectura optimista rápida (sin candado)
                            if (nueva_distancia < tabla_new[v].distance) 
                            {
                                // 2. ZONA CRÍTICA: Tomamos el mutex exclusivo del nodo destino
                                std::lock_guard<std::mutex> lock(*cerrojos[v]);
                                
                                // 3. Doble chequeo (DCLP): Verificar que otro hilo no haya mejorado 
                                // la distancia mientras esperabamos a que se abriera el candado
                                if (nueva_distancia < tabla_new[v].distance) {
                                    tabla_new[v].distance = nueva_distancia;
                                    tabla_new[v].predecessor = u;
                                    tabla_new[v].incoming_arc = arco;
                                    
                                    // Encendemos la bandera de forma thread-safe
                                    hubo_cambios.store(true, std::memory_order_relaxed);
                                }
                            }
                        }
                    }
            ...
            // Barrera de Sincronizacion manual: 
            // El hilo principal espera a que todos terminen su porcion antes de seguir
            for (auto& w : workers) {
                if (w.joinable()) {
                    w.join();
                }
            ...
            // Early exit
            if (!hubo_cambios.load(std::memory_order_relaxed)) break; 

            // Actualizamos la tabla base para la siguiente iteracion
            tabla_old = tabla_new;
        }

        // Deteccion de Ciclos Negativos (Secuencial)
        for (const auto& u : graph.getNodes()) {
            if (tabla_new[u].distance == INFINITO) continue;

            for (const auto& arco : graph.getOutgoingArcs(u)) {
                NodePtr v = arco->getTgtNode();
                if (v == u) v = arco->getSrcNode();
                
                if (tabla_new[u].distance + arco->getWeight() < tabla_new[v].distance) {
                    throw std::runtime_error("Fallo Critico: El grafo contiene un ciclo de peso negativo.");
                }
            }
        }

        // Construccion del SPT
        GraphType arbol_spt;
        std::unordered_map<NodePtr, NodePtr> mapa_nodos; 

        for (const auto& [nodo, info] : tabla_new) {
            if (info.distance != INFINITO) {
                NodePtr nuevo_nodo = arbol_spt.addNode(nodo->getValue());
                mapa_nodos[nodo] = nuevo_nodo;
            }
...
public:
    ...
    /**
     * @brief Calcula la distancia acumulada maxima desde la raiz en un arbol (como un SPT).
     * Ideal para el problema "Network Delay Time" (LeetCode 743).
     * @param tree El arbol de caminos minimos (devuelto por Dijkstra).
     * @param root El nodo origen desde donde empezo Dijkstra.
     * @param original_node_count La cantidad de nodos del grafo original.
     * @return std::pair<bool, WeightType> {true, max_distancia} si alcanzo todo, {false, 0} si hay inalcanzables.
     */
    static std::pair<bool, WeightType> getNetworkDelayTime(const GraphType& tree, NodePtr root, size_t original_node_count)
    {
        // 1. Validacion rapida: Si el arbol devuelto por Dijkstra tiene menos nodos 
        // que el grafo original, significa que hay servidores a los que nunca llego la señal.
        if (tree.getNodes().size() < original_node_count) {
            return {false, WeightType{}}; 
        }

        std::unordered_map<NodePtr, WeightType> distancias;
        std::queue<NodePtr> q;

        distancias[root] = WeightType{}; // Inicializamos la raiz en 0
        q.push(root);

        WeightType max_tiempo = WeightType{};

        // 2. BFS para sumar el tiempo acumulado
        while (!q.empty()) {
            NodePtr actual = q.front();
            q.pop();

            // En un SPT generado por tu Dijkstra, las flechas apuntan de la raiz hacia las hojas,
            // asi que solo necesitamos revisar las salidas.
            for (const auto& arco : tree.getOutgoingArcs(actual)) {
                NodePtr vecino = arco->getTgtNode();
                
                // Distancia acumulada = (Distancia desde la raiz al actual) + (Peso de este salto)
                WeightType nueva_distancia = distancias[actual] + arco->getWeight();
                distancias[vecino] = nueva_distancia;
                
                // Actualizamos el record del peor tiempo
                if (nueva_distancia > max_tiempo) {
                    max_tiempo = nueva_distancia;
                }
                
                q.push(vecino); // Seguimos bajando por el arbol
            }
        }

        return {true, max_tiempo};
...
/**
 * @brief Algoritmos de enrutamiento concurrente usando hilos crudos y semaforos/mutex.
 * Ideal para entender la sincronizacion clasica a nivel de sistema operativo.
 */
template <typename GraphType>
class ClassicConcurrentRouting
{
public:
    using NodePtr    = typename GraphType::NodePtr;
    using WeightType = typename GraphType::ArcWeightType;

    // Expandimos la estructura para saber de quien es el resultado, 
    // ya que los hilos terminaran en desorden.
    struct RoutingResult
    {
        ...
        bool success;
        std::vector<NodePtr> path;
        WeightType total_cost;
    };

    /**
     * @brief Calcula rutas en paralelo usando un pool manual de hilos y un Mutex.
     */
    static std::vector<RoutingResult> computeMultipleRoutes(
        const GraphType& graph,
        const std::vector<std::pair<NodePtr, NodePtr>>& tasks)
    {
        std::vector<RoutingResult> final_results;
        
        // Nuestro Semáforo Binario / Candado para proteger la escritura
        std::mutex mtx;

        // Contenedor para administrar los hilos de trabajo (Workers)
        std::vector<std::thread> workers;

        // 1. Lanzar los hilos
        for (const auto& task : tasks) 
        {
            // Creamos un hilo crudo pasandole una funcion lambda. 
            // Capturamos el grafo, el mutex y final_results por referencia (&), y la tarea por valor (=).
            workers.push_back(std::thread([&, task]() {
                NodePtr source = task.first;
                NodePtr target = task.second;

                // --- ZONA DE LECTURA (Paralela y sin bloqueos) ---
                // Cada hilo ejecuta Dijkstra en su propia porcion de memoria
                auto spt = Dijkstra<GraphType>::getMinimumPathsTree(graph, source);
                ...
                while (current != source && current != nullptr) {
                    ...
                    auto incoming = spt.getIncomingArcs(current);
                    ...
                    cost += incoming[0]->getWeight();
                    current = incoming[0]->getSrcNode();
                }

                // Empaquetamos el resultado local de este hilo
                RoutingResult local_result;
                local_result.source = source;
                local_result.target = target;
                ...
                if (current == source) {
                    ...
                    local_result.total_cost = cost;
                } else {
                    local_result.success = false;
                    local_result.total_cost = WeightType{};
                }

                // --- ZONA CRÍTICA (Protegida por el Mutex/Semaforo) ---
                // Solo un hilo a la vez puede entrar a estas llaves.
                {
                    std::lock_guard<std::mutex> lock(mtx); // El hilo toma el semaforo aqui
                    
                    // Como el push_back altera el tamaño del vector en memoria, 
                    // hacerlo sin candado causaria un colapso catastrofico.
                    final_results.push_back(local_result);
                    
                } // Al salir de las llaves, el hilo libera el semaforo automaticamente
            }));
        }

        // 2. Sincronización final (Barrera)
        // El hilo principal (main) se detiene aqui y espera a que todos los trabajadores terminen
...
/**
 * @brief Modulo para simulaciones avanzadas de concurrencia y grafos de asignacion de recursos.
 */
class ResourceAllocationSimulations
{
public:
    // Definimos un alias para la funcion de registro (Logger).
    // Asi el usuario decide si imprimir en consola, guardar en un archivo .txt, o ignorarlo.
    using LoggerFunc = std::function<void(const std::string&)>;

    /**
     * @brief Ejecuta la simulacion de los Filosofos Comensales libre de Deadlocks (Interbloqueos).
     * @param num_philosophers Cantidad de comensales (y por ende, de palillos).
     * @param target_meals Cuantas veces debe comer cada uno para declarar la simulacion exitosa.
     * @param logger Funcion lambda para manejar la salida de texto.
     */
    static void simulateDiningPhilosophers(int num_philosophers, int target_meals, LoggerFunc logger)
    {
        // Los mutex (palillos) NO se pueden copiar ni mover. 
        // Para meterlos en un vector de forma dinamica, usamos punteros inteligentes (unique_ptr).
        std::vector<std::unique_ptr<std::mutex>> chopsticks;
        for (int i = 0; i < num_philosophers; ++i) {
            chopsticks.push_back(std::make_unique<std::mutex>());
        }

        // Esta es la funcion que ejecutara cada hilo
        auto philosopher_task = [&](int id) {
            int left = id;
            int right = (id + 1) % num_philosophers;
            int meals = 0;

            while (meals < target_meals) {
                logger("[Filosofo " + std::to_string(id) + "] esta pensando...");
                std::this_thread::sleep_for(std::chrono::milliseconds(100)); // Tiempo pensando

                // Resolucion Asimetrica para evitar Deadlock
                if (id % 2 == 0) {
                ...
                meals++;
                logger("[Filosofo " + std::to_string(id) + "] COMIENDO (Comida " + 
                       std::to_string(meals) + "/" + std::to_string(target_meals) + ")");
                std::this_thread::sleep_for(std::chrono::milliseconds(150)); // Tiempo comiendo

                // Liberar recursos
                chopsticks[left]->unlock();
                chopsticks[right]->unlock();
            }
            logger(">>> [Filosofo " + std::to_string(id) + "] ESTA LLENO Y SE RETIRA.");
        };

        // Vector para agrupar los hilos
        std::vector<std::thread> threads;
        
        // Lanzamos la simulacion
        for (int i = 0; i < num_philosophers; ++i) {
            threads.push_back(std::thread(philosopher_task, i));
        }

        // Barrera de sincronizacion: El hilo principal espera a que todos terminen
        for (auto& t : threads) {
            if (t.joinable()) {
                t.join();
            }
...
/**
 * @brief Gestor de red dinamica con proteccion multihilo de Lector/Escritor.
 * Protege el grafo permitiendo multiples lecturas simultaneas o una escritura exclusiva.
 */
template <typename GraphType>
...
private:
    GraphType& graph; // Referencia al grafo original
    mutable std::shared_mutex rw_mutex; // El candado magico de C++17

public:
    explicit DynamicNetworkManager(GraphType& g) : graph(g) {}

    /**
     * @brief [ESCRITOR] Modifica el peso (latencia) de una arista.
     * Toma control EXCLUSIVO del grafo. Nadie puede leer ni escribir mientras esto ocurre.
     */
    void updateEdgeWeight(NodePtr u, NodePtr v, WeightType new_weight)
    {
        // unique_lock = Acceso Exclusivo (Bloquea a lectores y a otros escritores)
        std::unique_lock<std::shared_mutex> lock(rw_mutex);

        // Buscamos la arista y la actualizamos usando tu funcion lambda dinamica
        for (const auto& arco : graph.getOutgoingArcs(u)) {
            NodePtr destino = arco->getTgtNode();
            if (destino == u) destino = arco->getSrcNode(); // Proteccion self-loops

            if (destino == v) {
                arco->setWeightFunction([new_weight]() { return new_weight; });
                return; // Como usamos punteros compartidos, actualizar una instancia actualiza todas
            }
        }
        
        // Revisamos las entradas por si es un grafo dirigido y lo buscamos en reversa
        for (const auto& arco : graph.getIncomingArcs(u)) {
            NodePtr origen = arco->getSrcNode();
            if (origen == u) origen = arco->getTgtNode();
    ...
    /**
     * @brief [LECTOR] Calcula el Arbol de Expansion Minima (MST) de forma segura.
     * Toma un candado COMPARTIDO. Multiples hilos pueden hacer esto al mismo tiempo.
     */
    GraphType getSafeMST_Prim(NodePtr start) const
    {
        // shared_lock = Acceso Compartido (Solo bloquea a los escritores)
        std::shared_lock<std::shared_mutex> lock(rw_mutex);
        
        return Prim<GraphType>::getMinimumSpanningTree(graph, start);
    }
    
    /**
     * @brief [LECTOR] Calcula el MST usando Kruskal.
     */
    GraphType getSafeMST_Kruskal() const
    {
        std::shared_lock<std::shared_mutex> lock(rw_mutex);
        return Kruskal<GraphType>::getMinimumSpanningTree(graph);
    }
};
