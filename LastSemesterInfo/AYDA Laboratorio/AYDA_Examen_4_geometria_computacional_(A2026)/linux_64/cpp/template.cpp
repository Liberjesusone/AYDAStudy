...
Polygon build_secure_perimeter(const vector<Point2D>& fire_points, const vector<Point2D>& stations) {
    ...
    vector<Point2D> temporal = convex_hull_graham(fire_points);
    
    for(const auto &c : temporal)
    {
    	pandora.add_vertex(c);
    }
    
    for(const auto &c : stations)
    {		
    	if(is_inside(c, pandora))
  		{
    		throw std::runtime_error("LA_HECHICERA_COMPROMETIDA");
    	}
...
double calculate_buffer_zone_area(const Polygon& p_fire, const Vector2D& wind_vector) {
    ...
    p_fire.for_each_vertex([&](const Point2D& v)
    {
        all.push_back(v);
        all.push_back(Point2D(v.get_x() + wind_vector.get_x(), v.get_y() + wind_vector.get_y()));
    });
    
    vector<Point2D> _pandora = convex_hull_graham(all);
    Polygon _polygon;
    
    for(const auto& p : _pandora)
    {
        _polygon.add_vertex(p);
    }
    
    return compute_area(_polygon);
}
...
vector<Vector2D> optimize_camera_angles(
    ...
    vector<Vector2D> combo;
    int fire = -1;
    vector<Vector2D> current_combo;

    // Búsqueda de combinación
    auto solve = [&](auto& self, size_t dron_x) -> void {
    
        if(dron_x == camera_positions.size())
        {
            int count = 0;
            
            for(const auto& p : fire_points)
            {
                for(size_t i = 0; i < camera_positions.size(); ++i)
                {
                    Vector2D campo(camera_positions[i].get_x(), camera_positions[i].get_y());
                    VisionArea va = build_vision_area(campo, current_combo[i], radius, alpha);
                    if(is_inside(p, va))
                    {
                        count++;
                        break; 
