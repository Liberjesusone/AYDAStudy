...
namespace geom_lib
{
  ...
  public:
    ...
    double slope() const
    {
      ...
      return double(tgt_point.get_y() - src_point.get_y()) /
             double(tgt_point.get_x() - src_point.get_x());
    }
    ...
    bool is_perpendicular_with(const GenSegment &s) const
    {
      ...
      if (real_equal(ts, 0.))
      {
        return real_equal(ss, std::numeric_limits<double>::infinity()) || real_equal(ss, -std::numeric_limits<double>::infinity());
      }

      if (real_equal(ts, std::numeric_limits<double>::infinity()) || real_equal(ts, -std::numeric_limits<double>::infinity()))
    ...
    GenSegment get_perpendicular(const PointT &p) const
    {
      ...
      const typename PointT::NumberType x =
          double(y2 - y1 + m1 * x1 - m2 * x2) / double(m1 - m2);
    ...
    PointT intersection_with(const GenSegment &s) const
    {
      ...
      const double &m1 = slope();
      const double &m2 = s.slope();
