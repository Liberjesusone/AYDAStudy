...
namespace geom_lib
{
  ...
  double Vector2D::square_magnitude() const
  {
  ...
  void Vector2D::normalize()
  {
    ...
    if (real_equal(length, 0.) || real_equal(length, 1.))
      return;
  ...
  bool Vector2D::is_opposite(const Vector2D &v) const
  {
    ...
    return real_equal(a.x, -b.x) && real_equal(a.y, -b.y);
  }
  ...
  double Vector2D::angle_with(const Vector2D &v) const
  {
    return atan2(cross_product(v), dot_product(v));
  }

  void Vector2D::add_scaled_vector(const Vector2D &v, double scale)
  {
    x += v.x * scale;
    y += v.y * scale;
  }
  ...
  double Vector2D::scalar_product(const Vector2D &v) const
  {
  ...
  double Vector2D::vector_product(const Vector2D &v) const
  {
